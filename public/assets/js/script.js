...
if($('.owl-carousel.banner-sliderssurgery').length>0){var brandSlider=$('.banner-sliderssurgery');brandSlider.owlCarousel({loop:true,margin:0,dots:false,nav:false,autoplay:true,items:4.5,smartSpeed:500,center:true,responsive:{0:{items:1},500:{items:1},575:{items:2},768:{items:3},1000:{items:4.5},1300:{items:4.5}}})
...
$(window).scroll(function(){var sticky=$('.header'),scroll=$(window).scrollTop();if(scroll>=150)sticky.addClass('pharmacy-header');else sticky.removeClass('pharmacy-header');});$(window).on('load resize',function(){var window_width=$(window).outerWidth();var container_width=$('.container').outerWidth();var full_width=window_width-container_width
...
'</td>'+
'</tr>';$(".add-table-prescription").append(itemscontent);return false;});if($('.swiper-slider-banner').length>0){var swiper=new Swiper(".swiper-slider-banner",{effect:"cards",grabCursor:true,initialSlide:2,loop:false,rotate:true,mousewheel:{invert:false,},});}
...
$('.treatment-video a').on('click',function(){var currentVideo=$(this).parent().find(".doctor-treatment-video").get(0);var allVideos=$(".doctor-treatment-video");$(this).parent().find(".doctor-treatment-video").toggleClass('active');$(this).toggleClass('active');allVideos.each(function(){if(currentVideo!=this)
this.pause();});if(currentVideo.paused){currentVideo.play();}else{currentVideo.pause();}});$(function(){$("#table-filter").on("click",function(a){$(".form-sorts").toggleClass("table-filter-show");a.stopPropagation()});$(document).on("click",function(a){if($(a.target).is(".form-sorts")===false){$(".form-sorts").removeClass("table-filter-show");}});});$('.filter-dropdown-menu').click(function(event){event.stopPropagation();});if($('.select-img').length>0){function formatState(state){if(!state.id){return state.text;}
var $state=$('<span><img src="'+$(state.element).attr('data-image')+'" class="clinic-img" / " width="32px"> '+state.text+'</span>');return $state;};$('.select-img').select2({minimumResultsForSearch:Infinity,templateResult:formatState,templateSelection:formatState});}
if($('.select-social-img').length>0){function formatState(state){if(!state.id){return state.text;}
var $state=$('<span><img src="'+$(state.element).attr('data-image')+'" class="social-img" / " width="16px"> '+state.text+'</span>');return $state;};$('.select-social-img').select2({minimumResultsForSearch:Infinity,templateResult:formatState,templateSelection:formatState});}
if($('.flag-img').length>0){function formatState(state){if(!state.id){return state.text;}
var $state=$('<span><img src="'+$(state.element).attr('data-image')+'" class="flag-img" / " width="16px"> '+state.text+'</span>');return $state;};$('.flag-img').select2({minimumResultsForSearch:Infinity,templateResult:formatState,templateSelection:formatState});}
...
$(".add-social-media").on('click',function(){var servcontent='<div class="social-media-links d-flex align-items-center">'+
...
return false;});$(".start-appointment-set").on('click','.trash',function(){$(this).closest('.medication-wrap').remove();return false;});$(".add-medical").on('click',function(){var servcontent='<div class="col-md-12">'+
